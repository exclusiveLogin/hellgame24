...
export class UpdaterService {
  ...
  constructor(
    ...
    const timeout: number = this.state.getState().updateTimeout || 10000;
  ...
  private getAllSegments(): Observable<ISegment[]> {
    return this.http.get(`${this.api.getApi()}${this.path.segment}/${this.path.script}`, {
      ...
      tap( (seg: ISegment[]) => {
        ...
        seg.forEach( s => {
          const cached = this.segmentCache.find( cs => cs.id === s.id);
          if ( cached ) {
  ...
  private updateCache( segments: ISegment[]) {
    segments.forEach( (seg: ISegment) => {
      ...
      if ( target ) { target = seg; } else { this.segmentCache.push( seg ); }
